...
namespace DFWV
{
    public partial class TimelineForm : Form
    {
        World World;
        int lastEvent;
        List<XMLObject> WorldEvents = new List<XMLObject>();

        public List<string> CriticalEventTypes = new List<string>() { "created site", "entity created", "peace accepted", "created world construction", 
            "artifact created", "reclaim site", "destroyed site", "new site leader", "site taken over", "site abandoned"};

        public List<string> MajorEventTypes = new List<string>() {"hf died", "hf simple battle event", "created structure", "field battle", "attacked site", 
        ...
        public List<string> MinorEventTypes = new List<string>() { "create entity position", "merchant", "item stolen", "masterpiece lost", "change hf state", 
        ...
        public List<string> KnownEventTypes = new List<string>() { "create entity position", "merchant", "item stolen", "masterpiece lost", "hf simple battle event", "change hf state", 
        ...
        public List<string> UnknownEventTypes = new List<string>() {"agreement void", "site tribute forced", "hf destroyed site", "site died", "replaced structure", "hf razed structure", 
        ...
        public List<string> CriticalEventCollectionTypes = new List<string>() { "battle", "war", "site conquered" };

        public List<string> MinorEventCollectionTypes = new List<string>() { "abduction", "beast attack", "duel", "journey", "theft" };

        private Image stretchedMap;
        private Point PingLocation;
        private Image marker;
        private Image siteMarker;
        ...
        internal TimelineForm(World world)
        {
            InitializeComponent();
            World = world;
            ResourceManager resources = new ResourceManager("DFWV.Properties.Resources", typeof(TimelineForm).Assembly);
            marker = resources.GetObject("Marker") as Bitmap;
            siteMarker = resources.GetObject("SiteMarker") as Bitmap;
            ...
            Graphics g = Graphics.FromImage(stretchedMap);
        ...
        private void SortEvents()
        {
            ...
            foreach (HistoricalEvent he in World.HistoricalEvents.Values)
            {
        ...
        {
            XMLObject nextItem = GetNextItem();
        ...
        private void ModifyMap(XMLObject nextItem)
        {
            ...
            {
                HistoricalEvent evt = (HistoricalEvent)nextItem;
                
                Color penCol = Color.White;
                if (nextItem is HE_CreatedSite)
                    penCol = (nextItem as HE_CreatedSite).Civ.Civilization != null ?
                ...
                Graphics g = Graphics.FromImage(stretchedMap);
                Point loc = new Point();
                SizeF markerSize = new SizeF(7.0f,7.0f);
                if (siteSize.Width > 7)
            ...
            else if (nextItem is HE_CreatedWorldConstruction)
            {
                HE_CreatedWorldConstruction evt = (HE_CreatedWorldConstruction)nextItem;
                Graphics g = Graphics.FromImage(stretchedMap);
        ...
        private XMLObject GetNextItem()
        {
            ...
            if (lastEvent == WorldEvents.Count)
                return null;
            else 
                return WorldEvents[lastEvent];
        }
        ...
        private bool SkipEvent()
        {
            if (lastEvent < WorldEvents.Count && WorldEvents[lastEvent] is HE_CreatedWorldConstruction)
            {
                ...
                HE_CreatedWorldConstruction evt = (HE_CreatedWorldConstruction)WorldEvents[lastEvent];
                ...
                return lastEvent < WorldEvents.Count &&
                        ...
                        WorldEvents[lastEvent] is HistoricalEventCollection &&
                            (
                                chkEventCollectionCritical.Checked && CriticalEventCollectionTypes.Contains(HistoricalEventCollection.Types[((HistoricalEventCollection)WorldEvents[lastEvent]).Type])
                                ||
                                chkEventCollectionMinor.Checked && MinorEventCollectionTypes.Contains(HistoricalEventCollection.Types[((HistoricalEventCollection)WorldEvents[lastEvent]).Type])
                            )
                        ||
                        WorldEvents[lastEvent] is HistoricalEvent &&
                            (
                                chkEventCritical.Checked && CriticalEventTypes.Contains(HistoricalEvent.Types[((HistoricalEvent)WorldEvents[lastEvent]).Type])
                                ||
                                chkEventMajor.Checked && MajorEventTypes.Contains(HistoricalEvent.Types[((HistoricalEvent)WorldEvents[lastEvent]).Type])
                                ||
                                chkEventMinor.Checked && MinorEventTypes.Contains(HistoricalEvent.Types[((HistoricalEvent)WorldEvents[lastEvent]).Type])
                            )
                        );
        }
        ...
        private void lstEvents_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if (e.Index != -1)
            {
                ...
                if (lstEvents.Items[e.Index] is HistoricalEvent)
                {
                    Point loc = ((HistoricalEvent)lstEvents.Items[e.Index]).Location;
                    if (loc == Point.Empty)
                        drawstring = ((HistoricalEvent)lstEvents.Items[e.Index]).ToTimelineString();
                    else
                        drawstring = ((HistoricalEvent)lstEvents.Items[e.Index]).ToTimelineString() + " - (" + loc.X + "," + loc.Y + ")";
                    mColor = Color.Blue;
                }
                else
                {
                    HistoricalEventCollection hec = (HistoricalEventCollection)lstEvents.Items[e.Index];
                    
                    bool isHECending = false;
                    if (lstEvents.Items.IndexOf(hec) != e.Index)
                        isHECending = true;

                    Point loc = hec.Location;
                    if (loc == Point.Empty)
        ...
        private void HideMap()
        {
            ...
            this.Height = lstEvents.Bottom + 44;
        }
        ...
        private void DisplayMap()
        {
            ...
            picMap.Image = stretchedMap;
            this.Height = picMap.Bottom + 50;
        }

        private void lstEvents_SelectedIndexChanged(object sender, EventArgs e)
        {
            ListBox lstbox = (ListBox)sender;
            if (lstbox.SelectedItem is HistoricalEvent)
                Ping((HistoricalEvent)lstbox.SelectedItem);
            else
                Ping((HistoricalEventCollection)lstbox.SelectedItem);
        }

        private void Ping(HistoricalEventCollection evtCol)
        {
            if (evtCol.Location == Point.Empty)
                PingLocation = Point.Empty;
            else
                PingLocation = evtCol.Location;
            picMap.Refresh();
        }

        private void Ping(HistoricalEvent evt)
        {
            if (evt.Location == Point.Empty)
                PingLocation = Point.Empty;
            else
                PingLocation = evt.Location;
            picMap.Refresh();
        }
        ...
        private void picMap_Paint(object sender, PaintEventArgs e)
        {
            ...
            loc.X = (int)(PingLocation.X * siteSize.Width + siteSize.Width / 2 - marker.Width / 2);
            loc.Y = (int)(PingLocation.Y * siteSize.Height + siteSize.Height / 2 - marker.Height / 2);
        ...
        {
            List<int> Values = new List<int>() {10, 50, 100, 200, 500, 1000, 1500, 2000, 3000, 4000};
